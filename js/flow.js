...
function ch_d1(){
  aiSay([
    ...
    /* 【AI_Behavior_Instruction v1.1 §8.4a】三個分支各用不同的反應語開頭（這樣啊／了解／也是），
       不要每次都是同一句起手，讀起來才像真人聽到答案的當下反應 */
    const ch_d1Summary=val=>val==='一年以上'?'這樣啊，這筆資金的時間彈性較大，適合作中長期規劃，也有更大的空間參與市場成長':val==='一年內'?'了解，這筆資金隨時可能派上用場，會優先以「靈活性與安全性」為考量':'也是，這筆資金會採均衡配置，兼顧收益與調度彈性';
    const popover=renderComponent('popover/option-select',question,opts,opt=>{
...
function ch_d2(){
  /* 【AI_Behavior_Instruction v1.1 §9.11】不用「接下來想了解一下您的XX：」這種標題式起手，
     改成先講一句「為什麼想問」，實際問題交給下面 popover 的標題來問，避免同一句話講兩次 */
  aiSay(["波動有時候比想像中更難承受，想跟您確認一下實際能接受的程度。"],()=>{
    ...
    const opts=[
      /* 【AI_Behavior_Instruction v1.1 §6.5】小幅波動／明顯波動兩個分支後面還有 ch_d3 決定
         ...
         【§8.4a】兩個 ack 分別用「確實」「聽起來」起手，跟 ch_d1Summary 用的「這樣啊／
         了解／也是」不重複，同一段對話不會連續出現同一句反應語 */
      {label:'完全不能接受本金有任何虧損',val:'完全不能接受本金波動',
       ack:'*這代表本金安全是您最優先的考量，會以完全保本與高穩定的商品為主。*',
       next:()=>resolveConservative(),kw:['不能','保本','不要波動','不想虧','零風險','安全','不能虧','怕']},
      {label:'可以接受小幅波動，跌幅大概 10%～30% 都還好',val:'可接受小幅波動',
       ack:'*確實，您能接受一定程度的波動。*',
       next:()=>ch_d3(),kw:['小波動','可以接受','還好','一點點','小幅','ok','OK','接受','10%','20%','30%','跌幅']},
      {label:'可以接受明顯波動，跌幅超過 30% 也沒關係，只要能換到更好的成長機會',val:'可接受淨值明顯波動換取成長',
       ack:'*聽起來，您能接受較大幅度的波動，以換取成長機會。*',
...
function ch_d3(){
  /* 【AI_Behavior_Instruction v1.1 §9.11】不編號「最後一個問題」，改成直接講「為什麼想問」，
     實際問題交給下面 popover 的標題來問，避免同一句話講兩次；這裡不能先點名「債券／基金」，
     使用者這時還沒表達任何偏好，是資產類型由這一題的回答決定，不是使用者自己在債券／基金
     兩個選項間做選擇——先講出商品名稱等於預設了他還沒做的決定 */
  aiSay(["每個人看重的特質不同，適合的投資方向也會不一樣，想先了解您自己比較重視哪一種。"],()=>{
    const question="在投資型商品裡，您比較看重哪一種特質？";
    const opts=[
      {label:'比較希望領息穩定，到期時間也很明確',val:'領息穩定、到期時間明確',next:()=>resolveAttribute('bond','B'),
       kw:['領息','到期','穩定','固定','確定','債']},
      {label:'比較想定期定額分散風險，看重收益成長的潛力',val:'想以定期定額分散風險、追求收益潛能',next:()=>resolveAttribute('fund','A'),
       kw:['定期定額','分散風險','收益潛能','成長','基金','潛力']},
      {label:'兩種都可以，我還沒有特別的想法',val:'都可以／還沒決定',next:()=>resolveAttribute('combo','AB'),
